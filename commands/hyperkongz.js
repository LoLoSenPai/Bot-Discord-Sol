...
module.exports = {
	...
	async execute(interaction) {
		...
		if (collectionName === 'brozo') {
			...
			collectionAddress = '0xbede326c275727b97c716fe545717e1d9773156a';
		}
		...
		else if (collectionName === 'nine') {
			collectionAddress = '0xa9cc7c358bfa3fa6cee4231f975efc4ebd505538';
		}
		else {
		...
		try {
			...
			const response = await fetch(
				`https://polygonapi.nftscan.com/api/v2/assets/${collectionAddress}/${numb}?show_attribute=false`, {
				headers: {
			...
			const response2 = await fetch(`https://polygonapi.nftscan.com/api/v2/collections/${collectionAddress}?show_attribute=false`, {
				headers: {
					'X-API-KEY': 'UpHv9E2zqy9TValq6VhoRUXd'
				}
			});
			...
			// Get supply value from the second endpoint
			const supplyValue = data2.data.items_total;

			const rarityValue = data.data.rarity_rank;
			const name = data.data.name;
			const imageNft = data.data.image_uri;
			const urlNft = `https://magiceden.io/item-details/polygon/${contractAddress}/${numb}`
