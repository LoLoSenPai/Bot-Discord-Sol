    ...
    async execute(interaction) {
        ...
        if(target) {
            ...
            await channel.bulkDelete(filtered).then(messages => {
                res.setDescription(`${messages.size} messages supprimés avec succès depuis ${target}.`);
                interaction.reply({embeds: [res], ephemeral: true}); // you can use ephemeral if you desire
            });
        } else {
            await channel.bulkDelete(amount, true).then(messages => {
                res.setDescription(`${messages.size} messages supprimés avec succès depuis ce channel.`);
